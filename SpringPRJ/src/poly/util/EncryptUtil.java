...
public class EncryptUtil {
	...
	/*
	 * 암호와 알고리즘에 추가시킬 암호화 문구
	 * 
	 * 일반적인 암호화 알고리즘 SHA-256을 통해서만 암호화 시칼 경우, 암호화된 값만 보고 일반적인 비밀번호에 대한
	 * 값을 쉽게 예측이 가능함. 따라서, 암호화 할 때 
	 * 암호화되는 값에 추가적인 문자열을 붙여서 같이 암호화를 진행함.
	 * */
	 ...
	 //초기 벡터(16바이트 크기를 가지며, 16바이트 단위로 암호화 시, 암호화할 총 길이가 16바이트가 되지 못하면
	 //뒤에 추가하는 바이트)
	 ...
	 public static String encHashSHA256(String str) throws Exception {
		 ...
		 try {
			 ...
			 //여러 해시 알고리즘 중 가장 많이 사용되는 SHA-256 을 지원하고 있음.
