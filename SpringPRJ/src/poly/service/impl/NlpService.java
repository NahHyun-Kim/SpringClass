...
public class NlpService implements INlpService {
	...
	//NLP_DIC을 ㄱㄴㄷㄹ 순으로 나눠서 저장한 이유는 전체 약 몇만건의 데이터를 조회하는 것보다
	//일정한 범위를 지정하여 데이터 조회 횟수를 감소하기 위해서 나눔
	//가나다 순으로 저장될 데이터의 사전틀(가~하까지)
	private Map<String, List<NlpDTO>> NLP_DIC = new HashMap<String, List<NlpDTO>>();
	
	/**
	 * PostConstruct은 처음 한 번만 실행되도록 하는 어노테이션
	 * 데이터 사전 변수 생성을 위한 함수 최초 스프링 호출이 발생할 떄, 함수가 실행.
	 * 실행 이후 더 이상 실행은 하지 않는다.
	 * 
	 * NLP_DIC 변수에 값을 저장함
	 * */
	@Override
